    ...
    <div class=" w-full h-auto rounded-md">
      ...
      <div>
        <div class="w-full h-auto">
          <div class="w-full">
            <nuxt-img v-if="$auth.user.gender === 'male'" src="https://res.cloudinary.com/dhakagqld/image/upload/v1633068054/Whole%20Care%20Solutions/static/colors_hpvote.jpg" class="w-full h-24 md:h-64 object-cover bg-center overflow-hidden rounded-t-md" />
            <nuxt-img v-if="$auth.user.gender === 'female'" src="https://res.cloudinary.com/dhakagqld/image/upload/v1633068043/Whole%20Care%20Solutions/static/flowers_oecy7i.jpg" class="w-full h-24 md:h-64 object-cover bg-top overflow-hidden rounded-t-md" />
          </div>
          <div class="flex justify-center">
            <div class="w-full md:w-10/12 md:-mt-32 bg-white h-auto pb-16 md:rounded-lg shadow-lg p-4 relative">
              <div class="ml-4 -mt-16 md:ml-12 md:-mt-20 absolute">
                <nuxt-img class="rounded-full w-28 h-28 md:w-30 ring-white ring-4 object-cover inline-block border-white" :src="$auth.user.profileImage" />
              </div>
              <div class="mt-10 md:mt-14">
                ...
                <div>
                  <div class="md:ml-10 mt-12 grid">
                    ...
                    <span class="text-sm text-gray-400">{{ $auth.user.title }}</span>
                  </div>
                </div>
                    ...
                    <div class="text-gray-500 w-full md:grid md:grid-cols-2 md:px-5 py-5">
                      <div class="grid md:w-1/2 lg:w-1/4 space-y-3">
                        <div class="grid w-full">
                          ...
                          <span class="text-sm">{{ $auth.user.middleName }}</span>
                        </div>
                        <div class="grid w-full">
                          <span class="font-bold text-xs">Mobile Number</span>
                          <span class="text-sm">{{ '(+61)' + ' ' + $auth.user.mobile_number }}</span>
                        </div>
                        <div class="grid w-full">
                          <span class="font-bold text-xs">Home Number</span>
                          <span class="text-sm">{{ '(+61)' + ' ' + $auth.user.home_number }}</span>
                        </div>
                        <div class="grid w-full">
                          <span class="font-bold text-xs flex">Work Number</span>
                          <span class="text-sm">{{ '(+61)' + ' ' + $auth.user.work_number }}</span>
                        </div>
                      </div>
                      <div class="md:w-1/2 lg:w-1/4 mt-3 md:mt-0 space-y-3">
                        ...
                        <div class="grid w-full">
                          ...
                          <span class="text-sm">{{ $auth.user.gender }}</span>
                        </div>
                        <div class="grid w-full">
                          <span class="font-bold text-xs">Title</span>
                          <span class="text-sm">{{ $auth.user.title }}</span>
                        </div>
                      </div>
                    </div>
                    <div class="grid text-gray-500 w-full px-5">
                      <span class="font-bold text-xs">Bio</span>
                      <span class="text-sm">{{ $auth.user.bio }}</span>
                    </div>
                  </div>
                  <div v-if="tab === 2" class="text-gray-500 p-5">
                    <span class="p">Your schedule will be displayed here</span>
                  </div>
                  <div v-if="tab === 3" class="text-gray-500 p-5">
                    <span class="p">Your Qualifications will be displayed here</span>
                  </div>
                </div>
              </div>
            </div>
